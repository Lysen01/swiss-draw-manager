...
function bindEvents() {
  ...
  els.settingsForm.addEventListener("submit", (event) => {
    ...
    if (nextFormat === "round_robin" && maxRounds > 0 && nextRounds > maxRounds) {
      alert(`Для кругової системи з ${t.players.length} гравцями максимум ${maxRounds} турів.`);
      return;
    }
...
function normalizeRoundsCountForCurrentFormat(tournament) {
  const maxRounds = getMaxRoundsByFormat(tournament.format, tournament.players.length);
...
function renderTournamentTab() {
  ...
  els.roundMeta.textContent = `${t.name} | ${formatLabel(t.format)} | Тур ${t.currentRound} з ${t.roundsCount}`;
