...
router.get('/', async (_req, res, next) => {
  try {
    const result = await query(
      `SELECT id, name, city, contact, logo_url, created_at, updated_at
       FROM clubs
       ORDER BY name ASC`
    );
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const result = await query(
      `INSERT INTO clubs (id, name, city, contact, logo_url)
       VALUES (COALESCE($1::uuid, gen_random_uuid()), $2, $3, $4, $5)
       RETURNING id, name, city, contact, logo_url, created_at, updated_at`,
      [id, name, city, contact, logoUrl]
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const result = await query(
      `UPDATE clubs
       SET name = $2, city = $3, contact = $4, logo_url = $5, updated_at = NOW()
       WHERE id = $1
       RETURNING id, name, city, contact, logo_url, created_at, updated_at`,
      [id, name, city, contact, logoUrl]
    );
