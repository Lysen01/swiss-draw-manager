...
- перегляд і друк завершених турнірів без повторного редагування архіву.
...
Поточний основний продакшн-сервіс для тестування і деплою:
- `https://swiss-draw-manager-1.onrender.com/`
...
### 7.4 Завершений турнір після reload знову активний
- Перевір, що працюєш саме на `swiss-draw-manager-1.onrender.com`.
- При звичайному завершенні застосунок спочатку записує поточний турнір у PostgreSQL зі статусом `archived`, і тільки потім очищає поточний турнір.
- Якщо PostgreSQL не прийняв запис, користувач бачить помилку, а турнір не скидається мовчки.
